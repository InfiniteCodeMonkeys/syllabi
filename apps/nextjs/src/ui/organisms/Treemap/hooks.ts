...
export const useTreeMap = <TreeMapInputData>({
  ...
  const d3TreeMap = treemap<TreeMapInputData>()
    ...
    hierarchy(data)
      .sum((s: any) => s[valuePropInData as keyof typeof s])
      .sort(
        (a: any, b: any) =>
          b[valuePropInData as keyof typeof b] -
          a[valuePropInData as keyof typeof a],
      ),
  ) as unknown as CustomHierarchyRectangularNode<TreeMapInputData>;

...
export default function AcademyNode({
  data,
}: {
  ...
  const handleLike = () => {
    if (!isSignedIn) {
      ...
      if (!liked) {
        return removeLike.mutateAsync({ id: data.id });
      }
      saveLike.mutateAsync({ id: data.id });
    }
  };
  ...
  return (
    <>
      ...
      <div
        className={clsx(
          "rounded-md bg-gradient-to-r from-orange-600 to-pink-500",
          !expanded ? "w-68" : "w-88",
        )}
      >
        <div className="flex items-center justify-between p-2">
          <h4 className="mr-2 text-white">{data.name}</h4>
          <div className="mt-2">
            {!liked ? (
              <button
                type="button"
